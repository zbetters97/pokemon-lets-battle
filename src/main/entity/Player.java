...
/** END UPDATER **/
    ...
    public void walking() {
        ...
        if (canMove) {
            ...
            if (action == Action.SURFING) {
                if (gp.cChecker.checkGround(this)) {
                    action = Action.IDLE;
                    surfCounter = 0;
                    Progress.canSave = true;
    ...
    private void checkWildEncounter_Surfing() {
        // random encounter formula reference: https://bulbapedia.bulbagarden.net/wiki/Wild_Pok%C3%A9mon

        int r = new Random().nextInt(255);
        if (r < 5) {

            Pokemon wildPokemon = getWildPokemon(gp.wildEncounters_Surfing, gp.wildLevels_Surfing);
            ...
            for (final Pokemon[] pokemons : pcParty) {
                if (pokemons != null) {
                    for (final Pokemon pokemon : pokemons) {
                        if (pokemon != null && pokemon.getIndex() == index) {
                            ownsPokemon = true;
                            break;
                        }
                    }
