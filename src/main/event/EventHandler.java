...
public class EventHandler {
    ...
    public void checkEvent() {
        ...
        if (canTouchEvent) {
            ...
            // EVENT SPOTS
            if (hit(0, 30, 25, List.of("up"))) teleport(1, 22, 32, gp.shop, "up", false, false); // POKECENTER ENTRANCE
            else if (hit(1, 21, 32, "right", 2, List.of("down")))
                teleport(0, 30, 25, gp.town, "down", true, true); // POKECETER EXIT
            else if (hit(1, 22, 28, List.of("up"))) speak(gp.npc[1][0]); // NURSE JOY TALK
            else if (hit(2, 18, 27, List.of("left"))) speak(gp.npc[2][0]); // STORE CLERK TALK
            else if (hit(0, 35, 21, List.of("up")))
                teleport(2, 19, 31, gp.shop, "up", false, false); // POKEMART ENTRANCE
            else if (hit(2, 18, 31, "right", 2, List.of("down")))
                teleport(0, 35, 21, gp.town, "down", true, true); // POKEMART EXIT
        }
    }
